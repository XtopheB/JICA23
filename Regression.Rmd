...
The main focus here is on  regression models when we have many regressors. We will be trying to construct a "*good model*", through variable selection algorithms. 

xc<wx<w<ccwcwcwxxw

We will use an example where many regressors, possibly correlated, are used and we  will explore methods to construct the "*best*" set of variables. 
...
\, , 
\]
with possibly **many regressors** in $x$. 

> The intercept is always included in the model.

So we have a bunch of regressors, we would like to retain only those helpful in explaining $y$. 
...
We will look at a data set about the salary of households in a Bank. It includes a number of characteristics for each household, many of them are activity indicators and counts. The data frame contains  **`r nrow(BankAccount)`**  observations on the following **`r ncol(BankAccount)`** variables:
...
What if we center all variables? 
...
# From the library modelsummary we can create a nice summary table 
BankAccount %>% 
  select( Salary, Trans, HighTrans, Checks, Years) %>%
  datasummary_skim()
...
reglinsmall <- lm( Salary ~ Trans + HighTrans+ Checks + Years, data = BankAccount)

# results
summ(reglinsmall, center = FALSE)$coeftable %>%  
  kable(digits=3) %>% 
  kable_styling()
```
...
summ(reglinsmall, 
  ...
  kable_styling()
```
The $R^2$ for this model is `r  summary(reglinsmall)$adj.r.squared`
...
Is a measure of multicollinearity between variables. It indicates by how much the variance of the coefficient of $x_j$ is **inflated** due to the presence of other regressors. 

#### Definition

The VIF for regressor $x_j$ is calculated by 
...
In our example, omitting *Trans* (the Nb of transactions in 2020) does not change the fit of the model (adjusted $R^2$ was `r  round(summary(reglinsmall)$adj.r.squared, 3)` and is now  `r  round(summary(reglin2)$adj.r.squared, 3)`), nor the coefficients of the regressors not correlated with Trans. But the VIFs are now all small.

### Comparisons
```{r}
plot_summs(reglinsmall,reglin2,  scale = TRUE)
```
