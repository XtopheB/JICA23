...
output:
  ...
  word_document:
    #reference_docx: ../docs/KAZ-Template.docx
    toc: no 
...
```{r}
ggplot(data) +
  aes(x = education, y = income, colour = sex) +
  geom_point() +
  scale_color_hue(direction = 1) +
  labs(
    title = "My nice graphic",
    subtitle = "bla ",
    caption = "done in class"
  ) +
  theme_minimal() +
 facet_wrap(vars(sex))
...
ggplot(data, aes(x = sex, y = income, fill = sex)) +
  ...
  labs(
    title = "Income by Sex",
    subtitle = "Boxplot of income by sex",
    x = NULL,
    y = "Income"
  ) +
  coord_flip() +
...
## Let's try a Parallel Coordinate Plot

```{r}
library(GGally)
data %>%
  mutate(sexI = as.integer(sex)) %>%
  ggparcoord(
             columns     = c(7, 3, 1, 2),
             groupColumn = "sex",
             scale       = "uniminmax",
             alphaLines  = 0.1,
             mapping = ggplot2::aes(linewidth = 1.5, color = sex)) +
...
```{r, echo=TRUE}
modelMulti2 <- lm(income ~ education + sex +age_group , data = data)
summary(modelMulti2)
```
