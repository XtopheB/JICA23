...
# # The files are also on SIAP's server. You can use  the following approach (uncomment below) : 
# ## - Define the file location 
# my_temp_dir <- getwd()
# ## -  download the file 
# download.file("https://www.unsiap.or.jp/on_line/2023/TAPOS/Data/ne_10m_admin_0_countries/ne_10m_admin_0_countries.shp", destfile = file.path(my_temp_dir,"ne_10m_admin_0_countries.shp"))
# 
# ## - Read and store the file in shapefile object
# my_sf2 <- st_read(paste0(my_temp_dir,"/ne_10m_admin_0_countries.shp"))
...
my_geodata <- merge(my_sf_asia, my_sdg, by.x= "NAME_LONG", by.y = "GeoAreaName")%>%
  st_crop(c(xmin= 00, xmax = 520, ymin = -50,  ymax = 70))
...
## We have to transform the percentages into a categorical variable. 
my_geodata %>%
  mutate(
    my_class = cut(X2019, 
                   breaks = my_breaks,
...
```{r}
# Defining breakpoints from nb_classes
my_breaks <- quantile(my_geodata$X2019,
                      probs = seq(0, 1, length.out = nb_classes + 1),
                      na.rm = TRUE)
