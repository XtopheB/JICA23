...
# Example ggplot
ggplot(Mydata, aes(x = Population, y = Maternal, color = Country)) +
  geom_point(size = 3) +  # Use points to show data
  scale_x_continuous(labels = scales::comma) +  # Format x-axis labels for population
  theme_economist() +  # Use the economist theme
  scale_color_economist() +  # Economist color palette
  labs(
    title = "Population vs Maternal Health Metric by Country",
    x = "Population",
...
ggplot(Mydata) +
 aes(x = Maternal, y = Poverty, colour = Country) +
