---
title: "First exercise with TAPOS 2024"
author: "Christophe"
output:
  html_document:
    toc: yes
...
The data set contains   **`r  nrow(TradeData)`  observations**, and  `r ncol(TradeData)` variables    also I can do it like this ***`r Nvar `*** variables.
...
```{r}
library(ggplot2)
ggplot(TradeData, aes(y = Stat_Value)) +
  geom_boxplot() +
  labs(title = "Box Plot of stat_value",
       y = "stat_value") +
  coord_flip() +
  theme_minimal()
```
