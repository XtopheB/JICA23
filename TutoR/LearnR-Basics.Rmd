...
Now if I want to make it pretty, I can use the `paste()` funtion 
...
question("",
         answer("My age is 45",
                message = "Well, that would true be if ***Myage*** and ***myage***  are similar"),
         answer("My age is 25", message = " Well-done", correct = TRUE),
         answer("25",
                message = "No, We used `paste()` to heave a sentence"),
         allow_retry = TRUE
)
```

Try it by yourself and click on the `Run Code`  blue button. You can modify the code to change value, names, etc... 
...
vectors can be created  as a concantenation of numbers. We use The c()`  function to concatenate objects.
...
We dan have a summary using  `summary()` function 
...
We will use some SDG data that are preloaded in the `SDG` data frame
...
Then `%>% ` links operations in a very easy-to-read sequence. The code below does exactly the same opeartion as before, but using operation linking using  `%>% `
